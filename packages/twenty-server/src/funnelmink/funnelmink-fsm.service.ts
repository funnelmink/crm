...
export class FunnelminkFsmService {
  ...
  async addFunnelminkFSMObjectsToWorkspace(workspaceId: string): Promise<void> {
    ...
    try {
      await this.createFunnelminkObjects(workspaceId);
      await this.createFunnelminkRelations(workspaceId);
    } catch (error) {
      this.logger.error(
        `Error adding Funnelmink FSM objects to workspace: ${error.message}`,
        error.stack,
      );
      // Handle or log the error without breaking out of the process
    }

    // Further prefill data and views can be added here
  }

  private async createFunnelminkObjects(workspaceId: string): Promise<void> {
    for (const object of fsmObjects) {
      ...
      const objectMetadata = await this.objectMetadataService.createOne(input);
  ...
  private async createFunnelminkRelations(workspaceId: string): Promise<void> {
